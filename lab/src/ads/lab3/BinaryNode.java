...
public class BinaryNode<AnyType> {
    ...
    public int lowness(BinaryNode<AnyType> t) {
        if(t.isLeaf())
            return 0;
        if(left==null)
            return  1+lowness(right);
        if(right==null)
            return 1+lowness(left);

        return lowness(left)>lowness(right) ? 1+lowness(right): 1+lowness(left);
    }
    ...
    private int size(BinaryNode<AnyType> t) {
        if(t==null)
            return 0;
        return size(t.left)+t.size(t.right)+1;
    }
    ...
    private boolean shapely1(BinaryNode<AnyType> t) {
        return height(this)<=2*lowness(this);
    }
